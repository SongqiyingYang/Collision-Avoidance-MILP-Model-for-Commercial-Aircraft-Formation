...
while status == GRB.INFEASIBLE:
    ...
    T = 100.0 + ti #total time in seconds 
    ...
    A = 3 # number of formation aircraft
    ...
    vzmax = 0.010 # maximum vertical velocity (climb rate) 1k*ft/s 
    ...
    for ac in range(A): # join initial points for all aircraft except intruders 
                ...
                x0[1, ac] = sign*(offset[ac]*0.8*wingspan) 
                if D > 2: 
                    x0[2, ac] = 0.0
    
    print("x0 is:",x0) #ys test: initial point
    ...
    print("finalCourseWidth is:",finalCourseWidth) #ys test: final point       
    ...
    for iter in range(N):
        y[0,iter,0] = 20*(1+math.cos(intruder_radians)) + iter*delT*vy[0,0]  #need to be futher design
        y[1,iter,0] = 20*(math.sin(intruder_radians)) + iter*delT*vy[1,0]
        if D>2:
    ...
    t = m.addVars(D, N, A, lb=float('-inf'), ub=float('inf'), vtype=GRB.CONTINUOUS) # slack variable time for join/intruders 
    o = m.addVars(D, N, A, lb=float('-inf'), ub=float('inf'), vtype=GRB.CONTINUOUS) # slack variable for drag 
    dis = m.addVars(N-1, A, lb=float('-inf'), ub=float('inf'), vtype=GRB.CONTINUOUS) # slack variable for vertical maneuver distance
    ...
    for ac in range(A): #constraints for formation 
        for i in range(N): 
            m.addConstr((x[1, i, ac] == x0[1, ac])) 
            m.addConstr((v[1, i, ac] == v0[1, ac])) 
            m.addConstr((u[1, i, ac] >= u0[1, ac])) 
    ...
    for ac in range(A): #all aircraft are checked for final course constraints defined above in formation and intruder sections 
        for d in range(1, D): # along-course position doesn't matter, so only y and z dimensions are checked wrt final course constraints 
            for s in range(S): #sides, check both sides 
                if (s % 2) == 0: #if side is even, then check position of ac - position of ac2 
                    ...
                    sign = -1 
                m.addConstr((-sign*x[d, N-1, ac] <= finalCourseWidth[d, ac, s])) 
    ...
    # slack variable distance from final requirements constraints 
    ...
    m.addConstrs((x[0, i, 1] - x[0, i, 0] <= (x0[0,1] - x0[0,0]) + o[0, i, 1] for i in range(N)))
    m.addConstrs((-x[0, i, 1] + x[0, i, 0] <= (x0[0,1] - x0[0,0]) + o[0, i, 1] for i in range(N)))
    m.addConstrs((x[1, i, 1] - x[1, i, 0] <= (x0[1,1] - x0[1,0]) + o[1, i, 1] for i in range(N)))
    m.addConstrs((-x[1, i, 1] + x[1, i, 0] <= (x0[1,1] - x0[1,0]) + o[1, i, 1] for i in range(N)))
    m.addConstrs((x[2, i, 1] - x[2, i, 0] <= (x0[2,1] - x0[2,0]) + o[1, i, 1] for i in range(N)))
    m.addConstrs((-x[2, i, 1] + x[2, i, 0] <= (x0[2,1] - x0[2,0]) + o[1, i, 1] for i in range(N)))
    for ac in range(2,A):
        m.addConstrs((x[0, i, ac] - x[0, i, ac-2] <= (x0[0,ac] - x0[0,ac-2]) + o[0, i, ac] for i in range(N)))
        m.addConstrs((-x[0, i, ac] + x[0, i, ac-2] <= (x0[0,ac] - x0[0,ac-2])  + o[0, i, ac] for i in range(N)))
        m.addConstrs((x[1, i, ac] - x[1, i, ac-2] <= (x0[0,ac] - x0[0,ac-2])  + o[1, i, ac] for i in range(N)))
        m.addConstrs((-x[1, i, ac] + x[1, i, ac-2] <= (x0[0,ac] - x0[0,ac-2])  + o[1, i, ac] for i in range(N)))
        m.addConstrs((x[2, i, ac] - x[2, i, ac-2] <= (x0[0,ac] - x0[0,ac-2]) + o[2, i, ac] for i in range(N)))
        m.addConstrs((-x[2, i, ac] + x[2, i, ac-2] <= (x0[0,ac] - x0[0,ac-2]) + o[2, i, ac] for i in range(N)))
    for ac in range(1,A):
        for d in range (D): 
            m.addConstrs((o[d, i, ac] >= 0 for i in range(N))) 

    #slack variable for vertical maneuver distance
    for ac in range(A):
        m.addConstrs((x[2, i, ac] - x[2, i-1, ac] <= dis[i-1, ac] for i in range(1,N)))
        m.addConstrs((-x[2, i, ac] + x[2, i-1, ac] <= dis[i-1, ac] for i in range(1,N)))
        m.addConstrs((dis[i-1, ac] >= 0) for i in range(1,N))
    ...
    ro = 0.25 * np.ones(D) # drag weight 
    ro[2] = 1.0 #penalize vertical difference
    rdis = 1.0 #penalize vertical maneuver
    # Cost function 
    J_cost1 = gp.LinExpr() 
    J_cost2 = gp.LinExpr() 
    J_cost3 = gp.LinExpr() 
    ...
    for i in range(N):
        for ac in range(A): 
            for d in range(1,D): 
                J_cost2 += (rt[d]*t[d, i, ac]) #cost function for time elapsed until achieving final parameters along course at any x position 
            for d in range(D): 
                J_cost1 += (ru[d]*w[d, i, ac]) #cost fuction for acceleration used 
        for ac in range(1,A):
            for d in range(D):
                J_cost3 += (ro[d]*o[d, i, ac]) #cost fuction for drag
    
    for i in range(1, N):
        for ac in range(A):
            J_cost4 += (rdis*dis[i-1, ac]) #cost fuction for vertical manuveur
    
    J_cost = J_cost1 + J_cost2
